...
export class MovieService {
  ...
  static async getMovieTrailer(id: string) {
    ...
    const trailer = response.data.results.find((video: MovieTrailerType) => video.type === "Trailer" && video.site === "YouTube");
    return trailer?.key || null;
  }

  static async getSearchMovie(searchValue: string, filters: any, currentPage: number) {
